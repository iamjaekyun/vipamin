    ...
    def __init__(self, prompt_config, **kwargs):
        ...
        if self.prompt_config.INITIATION == "random":
            ...
            if self.prompt_config.DEEP:  # noqa
                vip = torch.load(f'match_moco_deep/{self.prompt_config.ID}_{self.prompt_config.K}')
                anti = torch.load(f'orth_moco_deep/{self.prompt_config.ID}')
                ...
                vip = torch.load(f'match_moco/{self.prompt_config.ID}_{self.prompt_config.K}', weights_only=True)
                anti = torch.load(f'orth_moco/{self.prompt_config.ID}', weights_only=True)
            ...
            if self.prompt_config.DEEP:
                spt = torch.load(f'spts_moco_deep/{self.prompt_config.ID}')
                ...
                spt = torch.load(f'spts_moco/{self.prompt_config.ID}', weights_only=True)
                self.prompt_embeddings = nn.Parameter(spt, requires_grad=True)
        else:
