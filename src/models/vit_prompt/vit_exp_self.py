...
class PromptedTransformer_EXPSELF(Transformer):
    def __init__(self, prompt_config, config, img_size, vis):
        ...
        elif self.prompt_config.INITIATION == "anti_comb":
            vip = torch.load(f'oracle_sup/{self.prompt_config.ID}_{self.prompt_config.K}', weights_only=True)
            anti = torch.load(f'anti_sup/{self.prompt_config.ID}', weights_only=True)
